...
public class ImpresionIETFiscalTest10 extends base{
	 @BeforeTest
	...
	@Test 
	public void impressionFiscal10() throws IOException, InterruptedException, AWTException {
		...
		Thread.sleep(2000);
		ld.TemplateName().sendKeys("Lisandro3");
		Thread.sleep(3000);
		...
		for(int i=0; i<til.size(); i++) {
			System.out.println(til.size());
			if(til.get(i).getText().contains("Horizontal")) {
				til.get(i).click();
				break;
			}
		}
		...
		for (int i=0; i<list.size();i++) {
			...
			if(list.get(i).getText().contains("Inferior Izquierda")) {
				list.get(i).click();
				break;
			}
